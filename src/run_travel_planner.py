...
def main():
    ...
    parser.add_argument(
        "--set_type", type=str, default="validation", choices=["validation", "test"]
    )
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--model_name", type=str, required=True)
    parser.add_argument(
        "--trace_path", type=str, default="results/traces/travel_planner"
    )
    ...
    for i, query_data in enumerate(data):
        ...
        intermediate_file = save_results(
            results[: i + 1], args.output_dir, args.set_type, args.model_name
        )
