...
def main():
    ...
    def process_task(task_info):
        ...
        if "question" in example:
            ...
            print(
                f"Found question: {task[:50]}..."
                if len(task) > 50
                else f"Found question: {task}"
            )

            # Extract answer if available
            answer = example.get("answer", "No answer provided")
            print(
                f"Found answer: {answer[:50]}..."
                if len(answer) > 50
                else f"Found answer: {answer}"
            )
        else:
        ...
        try:
            ...
            result = {
                ...
                if score_value.strip() in ["0", "1"]
                else 0,
                "task_id": task_id,
                "question": task[:200] + "..."
                if len(task) > 200
                else task,  # Include truncated question
            }
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = [
            executor.submit(process_task, (i, example)) for i, example in enumerate(ds)
        ]
        ...
        for future in tqdm(futures, desc="Waiting for tasks to complete"):
            ...
            if result and result.get("completed"):
                ...
                if result["score"] == 1:
                    ...
                    print(
                        f"File exists after writing: {os.path.exists(results_log_path)}"
                    )
